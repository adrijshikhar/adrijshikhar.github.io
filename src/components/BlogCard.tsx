...
export default function BlogCard({ title, date, href, description }: BlogCardProps) {
  return (
    ...
    <Card className="group relative h-full transition-colors duration-200 rounded-none border-0 border-b border-border bg-transparent p-0 py-5 shadow-none gap-2 hover:border-input focus-within:border-ring motion-reduce:transition-none">
      <CardHeader className="p-0 gap-2">
        ...
        <CardTitle className="text-heading text-xl leading-snug">
          <a
            href={href}
            className="group text-heading no-underline after:absolute after:inset-0 after:rounded-none after:content-[''] hover:text-heading hover:no-underline"
          >
